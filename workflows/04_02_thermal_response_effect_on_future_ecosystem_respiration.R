...
for (i in 1:length(files)) {
  ...
  for (iwindow in 1:nwindow) {
    ...
    while(nrow(data) < 100) {
      ...
      data <- a_measure_night_complete %>% filter(between(DOY, window_start - extend_days, window_end + extend_days))
  ...
  acclimation$R2[iacclimation] <- postResample(pred=ER_obs_pred$pred, obs=ER_obs_pred$obs)[2]
