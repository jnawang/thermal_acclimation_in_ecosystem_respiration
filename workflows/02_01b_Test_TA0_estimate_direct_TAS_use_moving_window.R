...
for (id in 1:nrow(site_info)) {
  ...
  if (! name_site %in% site_TS_issue) { next }
  ...
  if (name_site %in% site_TS_issue) {
    if (name_site == "GF-Guy") {
      # slope will be too low if using ac data for this tropical site. 
...
write.csv(outcome, file = file.path('data', 'outcome_temp_water_gpp_TA_issue.csv'), row.names = F)
write.csv(outcome_siteyear, file = file.path('data', 'outcome_siteyear_temp_water_gpp_TA_issue.csv'), row.names = F)
