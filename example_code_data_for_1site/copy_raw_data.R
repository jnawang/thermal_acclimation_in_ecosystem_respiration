...
files_FLUXNET2015  <- list.files(path=file.path(dir_rawdata, 'SiteData', 'FLUXNET2015'), pattern = "_FLUXNET2015_FULLSET_(HH|HR)_", full.names = TRUE)
files_FLUXNET2020  <- list.files(path=file.path(dir_rawdata, 'SiteData', 'FLUXNET2020'), pattern = "_FLUXNET2015_FULLSET_(HH|HR)_", full.names = TRUE)
...
files_ICOS2025 <- list.files(file.path(dir_rawdata, "SiteData", "Ecosystem final quality (L2) product in ETC-Archive format - release 2025-1"), pattern=".zip$", full.names = T)
