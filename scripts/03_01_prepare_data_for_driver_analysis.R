...
BIF.site <- BIF %>% filter(SITE_ID %in% site_info$site_ID) %>% filter(VARIABLE=='SOIL_CHEM_C_ORG') %>% group_by(SITE_ID) %>% summarise(soc_obs = mean(as.numeric(DATAVALUE), na.rm=T))     # 26 sites
...
# remove abnormal results; no need!
...
VI <- file1_interp %>% group_by(ID) %>% summarise(EVI=mean(EVI, na.rm=T), NDVI=mean(NDVI, na.rm=T)) 
# LAI: unitless
LAI <- file2_interp %>% group_by(ID) %>% summarise(Fpar=mean(Fpar, na.rm=T), LAI=mean(LAI, na.rm=T)) 
