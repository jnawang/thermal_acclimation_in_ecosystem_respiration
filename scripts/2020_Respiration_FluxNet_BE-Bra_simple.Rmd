...
a_2020 <- read.csv(file.path(exdir, file.in))
###############################################read in new data#######################
path_after2020  <- '/Users/jw2946/Documents/stability_project/SiteData/ICOS_after2020/'
files_after2020 <- list.files(path=path_after2020, pattern='.csv$', full.names = T)
#
a_after2020 <- read.csv(files_after2020[grepl(name_site, files_after2020)])
# check if the two datasets have similar data structure; no, a has more columns
# there is one year overlapping between the two datasets, so I need to remove that year
a <- bind_rows(a_2020[1:420768,], a_after2020)      # [17569:nrow(a_after2020), ]
#
# add more columns
...
#
  ...
  if (SWC) {
    plot(a$SWC_F_MDS_1, ylim=c(0,100))   # miss SWC in lots of years
  }
  plot(a$NEE_VUT_REF, ylim=c(-100,100))  # I should use QC=0, measured data
...
#  good_years
a_measure_night_complete <- a_measure_night_complete %>% filter(!YEAR %in% c(2003:2006, 2008, 2013, 2017, 2019, 2020))  # Spectral is noisy 2014, 2015. 
# change column names
...
data_dir <- '/Users/jw2946/Documents/stability_project/RespiarationData/FLUXNET2020/'
...
#    RMSE Rsquared      MAE     aicc
# 2.3594832 0.1533773 1.6791976                 # almost lineaer relationship with SWC
# 
# The best parameter values
# C_pool  1.1766140  0.0370800  31.732  < 2e-16 ***
# alpha   0.1103876  0.0056588  19.507  < 2e-16 ***
# beta   -0.0012389  0.0002393  -5.178 2.25e-07 ***
#
#-------------------------------------------------------------------------------------------------------------------
...
for (i in iStart:iEnd) {     
  data <- a_measure_night_complete %>% filter(YEAR==i)  # %>% filter(TS > 8)
  if (nrow(data) <= 10) {
    next
  }
  print(i)
...
cor.test(coef$beta, NEE_grow$TA)    # S
#
...
#  if (i==10) {next}
  ...
  if (perf$Rsquared[i] < 0.00) {             # I have to remove bad performance years. Year 2010 is also bad. 
    next
  }
  alpha  = coef$alpha[i]
  beta   = coef$beta[i]
  C_pool = coef$C_pool[i]
  result[((i-1)*nTS+1):(i*nTS), 2] <- C_pool * exp(alpha * TS + beta * TS * TS) # * 6 / (6 + 30)
}
...
cor.test(Ravg_GW, NEE_grow$TS)    # p-value = 0.0824;
cor.test(Ravg_GW, NEE_grow$TA)    # p-value = 0.774;  
#
# no correlation
...
# linear regression
summary(lm(data=df, alpha~TS))    # slope = -0.14747
...
# linear regression
summary(lm(data=df, alpha~TA))    # slope = 0.02232
