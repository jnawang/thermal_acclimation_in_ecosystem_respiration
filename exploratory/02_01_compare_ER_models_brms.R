...
# Junna Wang, Aug. 2025
#  It will take ~8 hours to finish running this script. 
...
for (id in 1:nrow(site_info)) {
  # id = 59  # nrow(site_info) # 1, 6, 77, 89, 64, 1:nrow(site_info)
  print(id)
...
# ----build a ER model for each site-year---- 
  ...
  priors_temp_water_gpp_temp <- priors_temp_water_gpp + 
    brms::prior("normal(0.05, 0.5)", nlpar = "gamma", lb = 0, ub = 0.2)
  
  # mod1
  # frmu[[1]]   <- NEE ~ C0 * exp(- alpha * (1/(15+46.02) - 1/(TS+46.02)))
  # stprm[[1]]  <- priors_temp1
  frmu[[1]]   <- NEE ~ exp(alpha * TS + beta*TS^2) * SWC / (Hs + SWC) * C0 + NEE_daytime3 * k2 * exp(gamma * TS)
  stprm[[1]]  <- priors_temp_water_gpp_temp
...
#    stprm[[4 + iwindow]] <- priors_temp_water_gpp_temp
    frmu[[4 + iwindow]] <- as.formula(paste0("NEE ~ exp(alpha * TS + beta*TS^2) * SWC / (Hs + SWC) * (C0 + k2 * ", varname, ")"))
    stprm[[4 + iwindow]] <- priors_temp_water_gpp
  }
  ...
  if ("GPP_DT" %in% names(ac)) {
    for (iwindow in 1:length(window_sizes)) {
      ...
      frmu[[4 + length(window_sizes) + iwindow]] <- as.formula(paste0("NEE ~ exp(alpha * TS + beta*TS^2) * SWC / (Hs + SWC) * (C0 + k2 * ", varname, ")"))
      stprm[[4 + length(window_sizes) + iwindow]] <- priors_temp_water_gpp
    }
  }
  ...
  # loop through each year: this takes too much time. 
  # randomly select two years for each site
  years_selected <- sample(unique(a_measure_night_complete$growing_year), 2)
  for (iyear in years_selected) {
    ...
    measurement_thresh <- min(100, nrow(data))  # we need to have at least 50 points for each window
    ...
    for (iday in 1:length(days)) {
      ...
      mod1 <- brms::brm(brms::bf(frmu[[1]], alpha+beta+C0+Hs+k2+gamma ~ 1, nl = TRUE),
                        prior = stprm[[1]], data = data_subset, iter = 1000, cores =4, chains = 4, backend = "cmdstanr",
    ...
    mod2 <- brms::brm(brms::bf(frmu[[2]], alpha+beta+C0 ~ 1, nl = TRUE),
              prior = stprm[[2]], data = data, iter = 1000, cores = 4, chains = 4, backend = "cmdstanr", 
              control = list(adapt_delta = 0.9, max_treedepth = 15), refresh = 0)
    # summary(mod2)
    ...
    if (site_info$SWC_use[id] == 'YES') {
      mod3 <- brms::brm(brms::bf(frmu[[3]], alpha+beta+C0+Hs ~ 1, nl = TRUE),
                        prior = stprm[[3]], data = data, iter = 1000, cores = 4, chains = 4, backend = "cmdstanr", 
                        control = list(adapt_delta = 0.9, max_treedepth = 15), refresh = 0)  # , silent = 2
      # summary(mod3)
    ...
    mod4 <- brms::brm(brms::bf(frmu[[4]], alpha+beta+C0+k2 ~ 1, nl = TRUE),
                      prior = stprm[[4]], data = data, iter = 1000, cores = 4, chains = 4, backend = "cmdstanr", 
                      control = list(adapt_delta = 0.9, max_treedepth = 15), refresh = 0)
    # print(summary(mod4))
    ...
    if (site_info$SWC_use[id] == 'YES') {
      for (ifrmu in 5:length(frmu)) {
        mod5 <- brms::brm(brms::bf(frmu[[ifrmu]], alpha+beta+C0+Hs+k2 ~ 1, nl = TRUE),
                          prior = stprm[[ifrmu]], data = data, iter = 1000, cores = 4, chains = 4, backend = "cmdstanr", 
                          control = list(adapt_delta = 0.9, max_treedepth = 15), refresh = 0)
...
result <- colMeans(ER_model_performance[!is.na(ER_model_performance$V30), -c(1:2)], na.rm=T)
df <- data.frame(RMSE = result[seq(1, 40, by=2)], R2 =result[seq(2, 40, by=2)])

write.csv(df, 'data/ER_model_comparison.csv', row.names = FALSE)
write.csv(ER_model_performance, 'data/ER_model_comparison_site_year.csv', row.names = FALSE)
