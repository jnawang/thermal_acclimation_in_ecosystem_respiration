...
```{r look at seasonal variation of TAS}
#
dir <- "/Users/junnawang/YaleLab/thermal_acclimation"
outcome_siteyear <- read.csv(file.path(dir, 'data/outcome_siteyear_window_temp_water_gpp.csv'))
site_info <- read.csv(file.path(dir, 'data/acclimation_data_new.csv'))


sites <- unique(outcome_siteyear$site_ID)
TAS <- data.frame()
for (site in sites) {
  outcome_site <- subset(outcome_siteyear, site_ID == site)
  df <- outcome_site %>% group_by(site_ID, window) %>%     
    summarise(
      TS_window = mean(TS),
      slope = {
        model <- lm(lnRatio ~ TS, data = cur_data())  # only current group
        coef(model)[2]
      }, .groups = "drop"
    )
  TAS <- rbind(TAS, df)
...
parts <- strsplit(TAS$window, '_')
TAS$DOY <- as.numeric(sapply(parts, `[`, 1))

# get IGBP and climate class
if (!'IGBP' %in% names(TAS)) {
  TAS <- TAS %>% left_join(site_info[,c('site_ID', 'IGBP', 'Climate_class')], by = 'site_ID')
}

climate_classes <- unique(site_info$Climate_class)
igbps <- unique(site_info$IGBP)

for (climate_class in climate_classes) {
  p1 <- TAS %>% filter(Climate_class == climate_class) %>%
  ggplot(aes(x = TS_window, y = slope, col = site_ID)) +
    geom_point() +
    geom_smooth(method = 'loess') +
    labs(title = climate_class) +
    theme_bw() + 
...
for (igbp in igbps) {
  p1 <- TAS %>% filter(IGBP == igbp) %>%
  ggplot(aes(x = TS_window, y = slope)) +  # , col = site_ID
    geom_point() +
    geom_smooth(method = "loess", se = FALSE) +  # method = "lm", formula = y ~ poly(x, 2), 
    labs(title = igbp) +
    lims(y = c(-0.2, 0.2)) +
    theme_bw() + 
    theme(legend.position = "none")
...
for (igbp in igbps) {
  p1 <- TAS %>% filter(IGBP == igbp) %>%
  ggplot(aes(x = DOY, y = slope )) +       #, col = site_ID
    geom_point() +
    geom_smooth(method = "loess", se = FALSE) +  # method = "lm", formula = y ~ poly(x, 2)
    labs(title = igbp) +
    lims(y = c(-0.2, 0.2)) +
...
```{r compare TAS by two methods }

dir <- "/Users/junnawang/YaleLab/thermal_acclimation"

TAS_test <- read.csv(file.path(dir, 'data/outcome_temp_water_gpp_test.csv'))
TAS <- read.csv(file.path(dir, 'data/outcome_temp_water_gpp.csv'))

plot(TAS_test$TAS, TAS$TAS)

cor.test(TAS_test$TAS, TAS$TAS)

sum(TAS_test$TASp < 0.05 & TAS_test$TAS < 0.0)   # 21 is positive and 22 is negative; 
sum(TAS$TASp < 0.05)

mean(TAS_test$TAS[which(TAS_test$TASp < 0.05 & TAS_test$TAS < 0.0)])  # -0.04716169


boxplot(TAS_test$TAS)
boxplot(TAS$TAS)
TAS$TAS_test <- TAS_test$TAS
summary(lm(data = TAS, TAS_test ~ TAS))
...
windows <- unique(outcome_siteyear$window)
outcome_siteyear %>% filter(site_ID == 'BR-Sa1') %>% 
  filter(extend_days < 80) %>% 
  filter(window %in% windows[10:21]) %>% 
  ggplot(aes(x=TS, y=lnRatio, col=window)) +
    geom_point() +
    geom_smooth(method = 'lm') +
    ylim(c(-0.9, 0.9)) +
    labs(title = outcome_siteyear$site_ID[nrow(outcome_siteyear)])

df_site_year_window <- outcome_siteyear %>% filter(site_ID == 'BR-Sa1') %>% 
  filter(extend_days < 80) %>%  filter(window %in% windows[10:21]) 
summary(lm(data=df_site_year_window, lnRatio ~ TS + window, na.action = na.exclude))$coefficients[2, c(1, 4)]

```


```{r }
data2sites <- read.csv('/Users/junnawang/Downloads/ERA5L_daily_2sites_1990_2024.csv')
dataAllsites <- read.csv('/Users/junnawang/YaleLab/data_server/ERA5_daily_swc_1990_2024_allsites.csv')

data2sites <- data2sites %>%
  # rename('SWC' = 'soil_moisture') %>%
  mutate(SWC = soil_moisture * 100)

df <- rbind(dataAllsites, data2sites[, c('date', 'name', 'SWC')])

plot(data2sites[data2sites$name == 'BR-Sa1', 'SWC'])
plot(data2sites[data2sites$name == 'BR-Ma2', 'SWC'])

write.csv(df, '/Users/junnawang/YaleLab/data_server/ERA5_daily_swc_1990_2024_allsites.csv' )
