# This script estimates direct thermal acclimation strength in ecosystem respiration using moving window methods.
# Authors: Junna Wang, August and September, 2025

# It takes about 20 minutes to finish the estimate of TAS. 
...
swc_ERA5 <- read.csv(file.path(dir_rawdata, "ERA5_daily_swc_1990_2024_1site.csv"))
...
write.csv(outcome, file = file.path(dir_rawdata, 'outcome_temp_water_gpp_1site.csv'), row.names = F)
write.csv(outcome_siteyear, file = file.path(dir_rawdata, 'outcome_siteyear_temp_water_gpp_1site.csv'), row.names = F)
