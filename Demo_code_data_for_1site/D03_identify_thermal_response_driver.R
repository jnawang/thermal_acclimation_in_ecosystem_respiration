...
# total TRS--strongly affected by both climate 
...
acclimation$Climate_reclass[acclimation$Climate_class %in% c("Am", "Af", "Cfa", "Cfb", "Cfc", "Dfa", "Dfb", "Dfc", "Dfd", "Dwc")] <- 'humid'
# create folds
k_folds <- 5
...
#####the model to use in manuscript
...
write.csv(partial_output, file = file.path(dir_rawdata, 'partial_plot.csv'), row.names = F)
write.csv(varImp_output, file = file.path(dir_rawdata, 'varImp_plot.csv'), row.names = F)
