...
#
acclimation <- read.csv(file.path(dir_rawdata, 'acclimation_data.csv'))
#
Tmin_month <- read.csv(file.path(dir_rawdata, 'Tmin_month_ssp245_wc.csv'))
...
#
files  <- list.files(path = dir_rawdata, pattern = '_ac.csv$', full.names = FALSE)
...
for (i in 1:length(files)) {
  ...
  #
  ac <- read.csv(file.path(dir_rawdata, files[i]))
  #
  # use only the years with qualified data
  a_measure_night_complete <- read.csv(file.path(dir_rawdata, paste0(name_site, "_nightNEE.csv")))
...
write.csv(acclimation, file=file.path(dir_rawdata, 'acclimation_data_future_ssp245_1site.csv'), row.names = F)
